          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {steps.map((step, index) => (
              <motion.div
                ...
                className="premium-card p-8 text-center group relative"
              >
                <div className="w-16 h-16 rounded-full border border-border mx-auto mb-6 flex items-center justify-center group-hover:border-foreground/30 transition-all duration-500 relative bg-background">
                  <step.icon className="w-6 h-6 text-muted-foreground group-hover:text-foreground transition-colors duration-500" />
                </div>
                <span className="text-[11px] tracking-[0.3em] uppercase text-muted-foreground/40 font-mono block mb-3">
                  {step.number}
                </span>
